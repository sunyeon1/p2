...
<div id="middle">	
	...
	<% 
		...
		ArrayList<Notice_VO> narray = Ndao.getAllBoard(startRow, endRow);
				 ...
				 <col width="70px" />
					...
					<tr style="height: 40px; border-bottom: 1px solid #ddd;" >
						...
						<% if(session.getAttribute("mname").equals("manager")) { %>
							<input type="button" value="글쓰기" onclick="location.href='Notice_D_Writeform.jsp'">
<% } %>
						</td>
					</tr>
			...
			for(int i=0;i < narray.size();i++){
				Notice_VO vo = narray.get(i);// 벡터에 저장되어있는 빈 클래스 하나씩 추출 
			%>
					<tr style="height: 30px; border-bottom: 1px solid #ddd;">
						...
						<td align="center" style =" text-decoration:none; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; padding-left: 10px; padding-right: 10px;">
							<a href= "Notice_D_Info.jsp?num=<%=number%>">
							<%=vo.getTitle()%>
							</a>
						</td>
						<td align="center"><%=vo.getId()%></td>
						<td align="center"><%=vo.getD()%></td>
						<td align="center"><%=vo.getCnt_ck1()%></td>
					</tr>
				<%}%>
		...
		<% 
			if(count > 0){
				...
				if(startPage > 10){
		%>				
					<a href="Notice_D_Listform.jsp?pageNum=<%=startPage-10%>">[이전]</a>
		<%			
				}	 
				//페이징 처리 
				for(int i=startPage;i<=endPage;i++){
		%>			
					<a href="Notice_D_Listform.jsp?pageNum=<%=i%>">[<%=i %>]</a>
		<% 			
				}
				//다음이라는 링크를 만들건지 파악
				if(endPage < pageCount){
		%>		
				<a href="Notice_D_Listform.jsp?pageNum=<%=startPage+10%>">[다음]</a>	
		<% 
				}	
			}
		%>
