...
<div class="aa">
	...
	<div class="intro"> <!-- 메인에서는 몰라도 메뉴에서는 사진 사이즈나 나오는 사진이 다르게 하기위해 이름변경 -->
		...
		<div class="header">
			...
			<div class="title"><a href = "../menu/index.jsp"><h1>The_Diner</h1></a></div> <!-- 타이틀 -->
...
<div id="middle">	
	...
	<% 
		...
		Question_VO vo = qdao.getOneBoard(num);
	%>
			...
			<table width = "100%">
				<tr style="height: 40px; border-bottom: 1px solid #ddd;">
					...
					<td width = "180" align ="center" style="border-left: 1px solid #ddd;"><%=vo.getCnt_ck2()%> </td>
				</tr>
				...
				<tr style="min-height: 400px; border-bottom: 1px solid #ddd;">
					...
					<td colspan = "3" align ="center" style=" padding-left: 50px; padding-right: 50px; border-left: 1px solid #ddd;"><textarea rows= "10" cols = "60" name = "content" style=" width: 100%; height: 320px; vertical-align: middle; " readonly="readonly"><%=vo.getContent3()%></textarea></td>
				</tr>
				
				<tr height = "40">
					<td align ="center" colspan = "4"> 
						<% if(vo.getId().equals(session.getAttribute("mname"))) { %>
						<input type="button" value="수정하기" onclick="location.href='Question_D_Updateform.jsp?num=<%=vo.getNum()%>'">
						<% } else if(session.getAttribute("mname") == null || session.getAttribute("mname").equals("manager")){ 
						...
						<% } %>	
						<% if(vo.getId().equals(session.getAttribute("mname"))) { %>
						<input type="button" value="삭제하기" onclick="location.href='Question_D_Deleteform.jsp?num=<%=vo.getNum()%>'"> 
						<% } else if(session.getAttribute("mname") == null || session.getAttribute("mname").equals("manager")){ 
						...
						<input type="button" value="목록보기" onclick="location.href='Question_D_Listform.jsp'">     
					</td>
				</tr>
		</table>
